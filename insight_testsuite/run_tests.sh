...
function compare_outputs {
  ...
  PROJECT_ANSWER_PATH5=${GRADER_ROOT}/temp/paymo_output/output5.txt
  ...
  TEST_ANSWER_PATH5=${GRADER_ROOT}/tests/${test_folder}/paymo_output/output5.txt  
  ...
  if [ "${DIFF_RESULT5}" -eq "0" ] && [ -f ${PROJECT_ANSWER_PATH5} ]; then
    echo -e "[${color_green}PASS${color_norm}]: ${test_folder} (output5.txt)"
    PASS_CNT=$(($PASS_CNT+1))
  else
    echo -e "[${color_red}FAIL${color_norm}]: ${test_folder} (output5.txt)"
...
function run_all_tests {
  ...
  NUM_TESTS=$(($(echo $(echo ${TEST_FOLDERS} | wc -w)) * 5))
  PASS_CNT=0
